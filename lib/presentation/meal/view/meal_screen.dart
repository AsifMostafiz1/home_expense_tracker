...
class MealScreen extends GetView<MealController> {
  ...
  void _showUserCalendarBottomSheet(
    ...
    // An admin's own row is edited the same way everybody else's is. Skipping
    // it used to leave them as the one member whose closed months nobody
    // could correct — the day-tap on the main calendar only reaches forward.
    final bool canAdminEdit = controller.isAdminUser;
    final MaterialColor accent = isMe ? Colors.teal : Colors.indigo;
    // "You" is a label, and the viewer's own card carries no phone — a write
    // needs the real pair, which the controller holds. Without both, the edit
    // affordances stay off rather than filing a row under an empty owner.
    final String targetName = isMe ? controller.currentUserName : userName;
    final String? ownerPhone =
        isMe ? controller.currentUserPhone : user['phone']?.toString();
    final String? memberPhone =
        (ownerPhone == null || ownerPhone.isEmpty || targetName.isEmpty)
            ? null
            : ownerPhone;

    // The member's own meal map, which the edit sheet reads the day's current
    // count out of — by phone, where the summary's per-day list can only be
    // matched on a name two people might share. Not awaited: the sheet opens
    // on what it already has and redraws when this lands.
                    ...
                    child: Column(
                      children: [
                        TableCalendar(
                          firstDay: c.firstDay,
                          lastDay: c.lastDay,
                          // Pinned to the month the summary was fetched
                          // for. The badges and the expense list below are
                          // that month's, and a calendar free to drift away
                          // from them read as a second, disagreeing one —
                          // an earlier month is reached from the screen
                          // behind, which moves the whole summary at once.
                          ...
                          onDaySelected: canAdminEdit && memberPhone != null
                              ? (selected, focused) {
                                  c.showAdminUpdateMealBottomSheet(
                                      selected, targetName, memberPhone);
                                }
                              : null,
                          headerStyle: HeaderStyle(
                            formatButtonVisible: false,
  ...
  void _confirmDeleteExpense(
    ...
    showConfirmDialog(
      ...
      message: isMe
          ? 'confirm_delete_my_expense'.tr
          : 'confirm_delete_expense_for'.trParams({'name': userName}),
